...
export default function LoginPage() {
  ...
  async function onSubmit(e: FormEvent) {
    e.preventDefault()
    if (!cmsEnabled()) {
      setError('Supabase env vars not configured. Set NEXT_PUBLIC_SUPABASE_URL and NEXT_PUBLIC_SUPABASE_ANON_KEY in Cloud Run.')
      return
    }
    ...
    <main className="flex min-h-screen items-center justify-center px-4 py-12">
      ...
      >
        <div className="mb-8 text-center">
          ...
          <h1 className="mt-4 text-xl font-bold text-gray-950">SunGene Admin</h1>
          <p className="mt-1 text-sm text-gray-500">Sign in to manage inquiries, testimonials, case studies, and videos.</p>
        </div>

        <label className="block">
          <span className="text-xs font-semibold uppercase tracking-wider text-gray-700">Email</span>
          <input
            type="email"
            value={email}
        ...
        <label className="mt-4 block">
          <span className="text-xs font-semibold uppercase tracking-wider text-gray-700">Password</span>
          <input
            type="password"
            value={password}
        ...
        <p className="mt-6 text-center text-xs text-gray-500">
          First time? Create an admin user in{' '}
          <a
