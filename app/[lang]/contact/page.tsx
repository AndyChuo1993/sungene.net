...
export async function generateMetadata({ params }: { params: Promise<{ lang: string }> }): Promise<Metadata> {
  ...
  return buildPageMetadata({
    ...
    pathname: '/contact',
    type: 'website',
    keywords: ['SunGene contact', 'request quotation', 'Taiwan China trading company inquiry', 'custom packaging supplier contact', 'home and living products supplier', 'outdoor products supplier', 'corporate gift supplier inquiry', 'Alibaba verified storefront contact', 'WhatsApp inquiry'],
  })
}
...
export default async function ContactPage({ params }: { params: Promise<{ lang: Lang }> }) {
  ...
  const content: Record<string, any> = {
    en: {
      ...
      desc: 'Send a reference image or competitor link, target quantity, and the market you sell into. We reply the same day during Taipei business hours (Mon–Fri, UTC+8) with a buyer-facing quotation, lead time, and the price band you should expect. MOQ USD 1,000 per shipment.',
      infoTitle: 'What to Include',
      formList: [
        'Product or category you need sourced',
        'Target quantity per shipment (e.g. 5,000 pcs, 1×20ft)',
        'Reference image, competitor SKU or Alibaba link',
        'Customization / branding / packaging requirements',
    ...
    cn: {
      kicker: '索取报价',
      title: '告诉我们您的需求',
      desc: '给我们一张参考图或竞品连结、目标数量、销往的市场。台北上班时间（周一至五，UTC+8）当天回讯——附上买方视角的报价、交期与您可以预期的价格区间。最低订单 USD 1,000 起接。',
      infoTitle: '请提供以下信息',
      formList: [
        '您加工或包装什么产品？',
        '每小时或每天的目标产量',
    ...
    zh: {
      kicker: '索取報價',
      title: '告訴我們你的需求',
      desc: '給我們一張參考圖或競品連結、目標數量、銷往的市場。台北上班時間（週一至五，UTC+8）當天回訊——附上買方視角的報價、交期與你可以預期的價格區間。最低訂單 USD 1,000 起接。',
      infoTitle: '請提供以下資訊',
      formList: [
        '您加工或包裝什麼產品？',
        '每小時或每天的目標產量',
    ...
    fr: {
      ...
      desc: "Envoyez une photo de référence ou un lien concurrent, la quantité cible et le marché visé. Réponse le jour même pendant les heures de bureau de Taipei (lun–ven, UTC+8) avec un devis côté acheteur, le délai et la fourchette de prix attendue. Commande minimum USD 1 000 par expédition.",
    ...
    es: {
      kicker: 'SOLICITAR UNA COTIZACIÓN',
      title: 'Cuéntenos qué necesita',
      desc: 'Envíe una foto de referencia o enlace de competencia, cantidad objetivo y el mercado al que vende. Respondemos el mismo día en horario laboral de Taipéi (lun–vie, UTC+8) con una cotización del lado comprador, plazo de entrega y el rango de precios esperado. Pedido mínimo USD 1 000 por envío.',
      infoTitle: 'Información a incluir',
      formList: ['¿Qué producto procesa o empaca?', 'Cantidad objetivo por envío (ej. 5.000 pzs, 1×20ft)', 'Formato de empaque (bolsas, botellas, pouches, etc.)', 'Personalización / marca / empaque', 'Mercado de destino / país de envío', 'Rango de presupuesto (opcional)'],
    ...
    pt: {
      ...
      formList: ['Produto ou categoria a fazer sourcing', 'Quantidade alvo por envio (ex. 5.000 pçs, 1×20ft)', 'Imagem de referência, SKU concorrente ou link Alibaba', 'Personalização / marca / embalagem', 'Mercado de destino / país de envio', 'Faixa de orçamento (opcional)'],
              ...
              <Card className="p-8 shadow-elev-2">
                <InquiryForm
                  ...
                  fields={[
                    ...
                    { name: 'product', label: ({ en: 'Product / Category', cn: '产品 / 品类', zh: '產品 / 品類', fr: 'Produit / Catégorie', es: 'Producto / Categoría' } as Record<string,string>)[lang] || 'Product / Category', type: 'text', required: true, placeholder: ({ en: 'e.g., stand-up pouches, glass jars, garden tools, airless pumps...', cn: '例如：立袋、玻璃罐、园艺工具、真空泵...', zh: '例如：立袋、玻璃罐、園藝工具、真空泵...', fr: 'ex. pochettes stand-up, bocaux verre, outils jardin, pompes airless...', es: 'ej. bolsas stand-up, frascos de vidrio, herramientas de jardín, bombas airless...' } as Record<string,string>)[lang] || '' },
                    { name: 'capacity', label: ({ en: 'Quantity per shipment', cn: '每批数量', zh: '每批數量', fr: 'Quantité par expédition', es: 'Cantidad por envío' } as Record<string,string>)[lang] || 'Quantity per shipment', type: 'text', placeholder: ({ en: 'e.g., 5,000 pcs, 1 x 20ft container, monthly reorder', cn: '例如：5,000 件、1 个 20 呎货柜、每月补单', zh: '例如：5,000 件、1 個 20 呎貨櫃、每月補單', fr: 'ex. 5 000 pces, 1 x 20\' conteneur, réassort mensuel', es: 'ej. 5 000 pzs, 1 contenedor 20", reabastecimiento mensual' } as Record<string,string>)[lang] || '' },
                    { name: 'country', label: ({ en: 'Destination Country', cn: '目标国家', zh: '目標國家', fr: 'Pays de destination', es: 'País de destino' } as Record<string,string>)[lang] || 'Destination Country', type: 'text' },
                    { name: 'reference', label: ({ en: 'Reference image / URL (optional)', cn: '参考图 / 链接（选填）', zh: '參考圖 / 連結（選填）', fr: "Image de référence / URL (facultatif)", es: 'Imagen de referencia / URL (opcional)' } as Record<string,string>)[lang] || 'Reference image / URL (optional)', type: 'text', placeholder: ({ en: 'Competitor product link, or describe the look you want', cn: '竞品链接，或描述您想要的样式', zh: '競品連結，或描述你想要的樣式', fr: 'Lien produit concurrent, ou description du visuel souhaité', es: 'Enlace de producto competidor, o descripción del estilo que busca' } as Record<string,string>)[lang] || '' },
