...
export async function generateMetadata({ params }: { params: { lang: Lang } }): Promise<Metadata> {
  ...
  return {
    title: lang === 'zh' ? '免費索取海外經銷商名單｜SunGene' : 'Get a Distributor List (Free) | SunGene',
    description:
      lang === 'zh'
    ...
    alternates: { canonical: `/${lang}/distributor-list`, languages: { zh: '/zh/distributor-list', en: '/en/distributor-list' } },
  }
}
    ...
    <main className="pt-28">
      <div className="mx-auto max-w-6xl px-6">
        <div className="grid gap-10 lg:grid-cols-2 items-start">
          <div>
            <h1 className="text-4xl font-bold tracking-tight text-gray-900">{isZh ? '免費索取海外經銷商/代理名單' : 'Get an International Distributor List (Free)'}</h1>
            ...
            <div className="mt-8 grid gap-4">
              {[
                isZh ? '依國家/區域與通路角色分層' : 'Segmented by country/region and channel roles',
                isZh ? '提供初步篩選建議（適合/不適合原因）' : 'Includes initial vetting notes (fit / non-fit)',
                isZh ? '可延伸到合作 Offer 與跟進節奏設計' : 'Extend into partnership offer and follow-up cadence',
              ].map((t, i) => (
