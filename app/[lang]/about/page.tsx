...
export async function generateMetadata({ params }: { params: Promise<{ lang: string }> }): Promise<Metadata> {
  ...
  const descriptions: Record<string, string> = {
    en: 'A small trading company with offices in Taichung, Taiwan and mainland China. We buy from vetted factories and resell direct to international buyers — packaging, home goods, garden products. Three years on Alibaba.com.',
    cn: '一家在台湾台中与中国大陆均设有公司的小型贸易商。我们直接向核查过的工厂下单、转手出货给海外买家——包装产品、家居用品、园艺产品。Alibaba.com 经营三年。',
    zh: '一家在台灣台中與中國大陸均設有公司的小型貿易商。我們直接向審查過的工廠下單、轉手出貨給海外買家——包裝產品、家居用品、園藝產品。Alibaba.com 經營三年。',
    fr: "Petite société de négoce avec bureaux à Taichung (Taïwan) et en Chine continentale. Nous achetons auprès d'usines vérifiées et revendons en direct aux acheteurs internationaux — produits d'emballage, articles de maison, jardin. Trois ans sur Alibaba.com.",
    es: 'Pequeña empresa comercial con oficinas en Taichung (Taiwán) y China continental. Compramos a fábricas verificadas y revendemos directamente a compradores internacionales — empaque, hogar, jardín. Tres años en Alibaba.com.',
...
export default async function AboutPage({ params }: { params: Promise<{ lang: Lang }> }) {
  ...
  const content: Record<string, any> = {
    en: {
      ...
      title: 'A trading company built so quality is our own problem',
      intro: 'We are SunGene — two registered companies, one in Taichung, Taiwan and one in mainland China. We buy from vetted factories and resell directly to international buyers in packaging, home goods, and garden categories. Background: three years operating as a verified Alibaba.com supplier at momas.en.alibaba.com, plus prior B2B sales careers before that.',
      mission: 'We chose the trader model — not the commission-agent model — for a specific reason: it puts the goods on our balance sheet. That single accounting fact changes the incentive structure. Rejecting a sub-spec batch becomes our own financial decision, not a courtesy we extend to you. Quality moves from "we will try" to "we have to".',
      stats: [
        { value: 'TW + CN', label: 'Offices' },
        { value: '3 yr', label: 'On Alibaba.com' },
        { value: 'USD 1,000', label: 'Min. order' },
        { value: '0', label: 'Kickbacks accepted' },
      ],
      strengthsTitle: 'How we differ from a typical sourcing agent',
      strengths: [
        { title: 'Direct buy, not commission', desc: 'Most agents earn a commission on the factory price you never see. We do not. We buy from the factory at FOB or EXW, then resell to you with our margin disclosed on top. You can ask us the underlying factory price at any time.' },
        { title: 'On-site QC by our own team', desc: 'Many agents subcontract inspection to people who never set foot in the factory. We drive to the factory ourselves for Taiwan suppliers, or have the goods routed through our forwarder’s warehouse for China suppliers, where we inspect before export. Inspection photos and video accompany every shipment.' },
        { title: 'Honest about scope', desc: 'We are not pretending to be a 50-country veteran. We have a working supplier list in packaging, home goods, and garden products today. For adjacent categories we say so plainly, and only take the work if we can commit to delivering it.' },
      ],
      ctaTitle: 'Try a first small order',
      ctaDesc: 'USD 1,000 minimum per shipment. Send us a reference image, your target quantity, and the market you sell into. We reply the same business day with two or three factory directions and the rough price band you should expect.',
      ctaBtn: 'Start a sourcing conversation',
    },
    cn: {
      kicker: '关于 SunGene',
      title: '一家把品质当成自己问题的贸易公司',
      intro: '我们是 SunGene——两家注册公司，一家在台湾台中，一家在中国大陆。我们直接向核查过的工厂买进、转手卖给海外买家，集中在包装、家居与园艺品类。背景：在 Alibaba.com 上以 momas.en.alibaba.com 这个认证供应商身分经营三年，之前都是做 B2B 业务出身。',
      mission: '我们选「贸易商」而不是「抽佣代理」这个模式，理由很具体：贸易商模式让货走我们自己的帐。这一个会计事实就改变了诱因结构——挡下一批不合规的货，是我们自己的财务决定，不是我们给您的人情。品质从「我们尽量」变成「我们不得不」。',
      stats: [
        { value: '台湾＋中国', label: '双公司' },
        { value: '3 年', label: 'Alibaba.com' },
        { value: 'USD 1,000', label: '最低订单' },
        { value: '0', label: '红包接受次数' },
      ],
      strengthsTitle: '我们跟一般采购代理有什么不同',
      strengths: [
        { title: '直接买进卖出，不抽佣', desc: '大部分代理是按工厂报价抽佣——而工厂报价您看不到。我们不抽佣。我们在 FOB 或 EXW 把货买下，加上揭露在外面的转售利润后卖给您。您随时可以问底下的工厂报价。' },
        { title: '团队亲自验货', desc: '不少代理把验货外包给从没踏进工厂的人。我们台湾的工厂自己开车去。中国的工厂出货先进我们合作货代的仓库，由我们验货后才放行出口。每批附验货照片与影片。' },
        { title: '诚实说清楚做得到什么', desc: '我们不会装成「服务过 50 国老司机」。我们目前在包装、家居、园艺这三块有稳定供应商名单。相邻品类我们会直说，能承诺交付才接案。' },
      ],
      ctaTitle: '先用一笔小单试试',
      ctaDesc: '最低 USD 1,000 起接。给我们一张参考图、目标数量、销往的市场，我们当个上班日回讯——附上我们会筛出来的 2–3 家工厂方向，以及您可以预期的价格区间。',
      ctaBtn: '开始一次采购对话',
    },
    zh: {
      kicker: '關於 SunGene',
      title: '一家把品質當成自己問題的貿易公司',
      intro: '我們是 SunGene——兩家註冊公司，一家在台灣台中，一家在中國大陸。我們直接向審查過的工廠買進、轉手賣給海外買家，集中在包裝、家居與園藝品類。背景：在 Alibaba.com 上以 momas.en.alibaba.com 這個認證供應商身分經營三年，之前都是做 B2B 業務出身。',
      mission: '我們選「貿易商」而不是「抽佣代理」這個模式，理由很具體：貿易商模式讓貨走我們自己的帳。這一個會計事實就改變了誘因結構——擋下一批不合規的貨，是我們自己的財務決定，不是我們給你的人情。品質從「我們盡量」變成「我們不得不」。',
      stats: [
        { value: '台灣＋中國', label: '雙公司' },
        { value: '3 年', label: 'Alibaba.com' },
        { value: 'USD 1,000', label: '最低訂單' },
        { value: '0', label: '紅包接受次數' },
      ],
      strengthsTitle: '我們跟一般採購代理有什麼不同',
      strengths: [
        { title: '直接買進賣出，不抽佣', desc: '大部分代理是按工廠報價抽佣——而工廠報價你看不到。我們不抽佣。我們在 FOB 或 EXW 把貨買下，加上揭露在外面的轉售利潤後賣給你。你隨時可以問底下的工廠報價。' },
        { title: '團隊親自驗貨', desc: '不少代理把驗貨外包給從沒踏進工廠的人。我們台灣的工廠自己開車去。中國的工廠出貨先進我們合作貨代的倉庫，由我們驗貨後才放行出口。每批附驗貨照片與影片。' },
        { title: '誠實說清楚做得到什麼', desc: '我們不會裝成「服務過 50 國老司機」。我們目前在包裝、家居、園藝這三塊有穩定供應商名單。相鄰品類我們會直說，能承諾交付才接案。' },
      ],
      ctaTitle: '先用一筆小單試試',
      ctaDesc: '最低 USD 1,000 起接。給我們一張參考圖、目標數量、銷往的市場，我們當個上班日回訊——附上我們會篩出來的 2–3 家工廠方向，以及你可以預期的價格區間。',
      ctaBtn: '開始一次採購對話',
    },
    fr: {
      kicker: 'À PROPOS DE SUNGENE',
      title: "Une société de négoce conçue pour que la qualité soit notre propre problème",
      intro: "SunGene, c'est deux sociétés enregistrées — une à Taichung (Taïwan) et une en Chine continentale. Nous achetons auprès d'usines vérifiées et revendons en direct aux acheteurs internationaux dans les catégories emballage, maison et jardin. Historique : trois ans en tant que fournisseur Alibaba.com vérifié sous momas.en.alibaba.com, après plusieurs années de carrière commerciale B2B.",
      mission: "Nous avons choisi le modèle négociant — pas le modèle agent à la commission — pour une raison précise : la marchandise apparaît à notre bilan. Ce simple fait comptable change la structure des incitations. Refuser un lot hors spécification devient notre propre décision financière, pas une courtoisie que nous vous accordons. La qualité passe de « nous essaierons » à « nous devons ».",
      stats: [
        { value: 'TW + CN', label: 'Bureaux' },
        { value: '3 ans', label: 'Sur Alibaba.com' },
        { value: 'USD 1 000', label: 'Commande min.' },
        { value: '0', label: 'Pourboires acceptés' },
      ],
      strengthsTitle: "En quoi nous différons d'un agent de sourcing classique",
      strengths: [
        { title: "Achat-revente, pas de commission", desc: "La plupart des agents perçoivent une commission sur un prix usine que vous ne voyez pas. Pas nous. Nous achetons à l'usine en FOB ou EXW, puis nous revendons avec notre marge clairement affichée. Le prix usine sous-jacent est communiqué sur simple demande." },
        { title: 'Contrôle qualité par notre équipe', desc: "Beaucoup d'agents sous-traitent l'inspection à des intervenants qui ne mettent jamais les pieds à l'usine. Nous nous déplaçons nous-mêmes pour les fournisseurs taïwanais ; pour la Chine, la marchandise transite par l'entrepôt de notre transitaire où nous inspectons avant export. Photos et vidéo à chaque expédition." },
        { title: 'Honnêteté sur le périmètre', desc: "Nous ne nous présentons pas comme vétérans en 50 pays. Aujourd'hui, nous avons une liste de fournisseurs active sur l'emballage, la maison et le jardin. Pour les catégories voisines, nous le disons franchement et n'acceptons le travail que si nous pouvons nous y engager." },
      ],
      ctaTitle: 'Essayez une première petite commande',
      ctaDesc: 'Minimum USD 1 000 par expédition. Envoyez-nous une image de référence, votre quantité cible et le marché visé. Réponse le jour même avec 2–3 directions usines et la fourchette de prix attendue.',
      ctaBtn: 'Démarrer une conversation sourcing',
    },
    es: {
      kicker: 'SOBRE SUNGENE',
      title: 'Una empresa comercial diseñada para que la calidad sea nuestro propio problema',
      intro: 'SunGene son dos empresas registradas — una en Taichung (Taiwán) y otra en China continental. Compramos a fábricas verificadas y revendemos directamente a compradores internacionales en categorías de empaque, hogar y jardín. Historial: tres años como proveedor verificado en Alibaba.com bajo momas.en.alibaba.com, tras varios años de carrera en ventas B2B.',
      mission: 'Elegimos el modelo comercial — no el de agente a comisión — por una razón concreta: la mercancía aparece en nuestro balance. Ese único hecho contable cambia la estructura de incentivos. Rechazar un lote fuera de especificación pasa a ser nuestra propia decisión financiera, no una cortesía hacia usted. La calidad cambia de "lo intentaremos" a "tenemos que".',
      stats: [
        { value: 'TW + CN', label: 'Oficinas' },
        { value: '3 años', label: 'En Alibaba.com' },
        { value: 'USD 1 000', label: 'Pedido mín.' },
        { value: '0', label: 'Sobres aceptados' },
      ],
      strengthsTitle: 'En qué nos diferenciamos de un agente de sourcing típico',
      strengths: [
        { title: 'Compra-reventa, no comisión', desc: 'La mayoría de los agentes cobra una comisión sobre un precio de fábrica que usted no ve. Nosotros no. Compramos en FOB o EXW a la fábrica y revendemos con nuestro margen mostrado en la cotización. El precio de fábrica subyacente se comunica bajo petición.' },
        { title: 'Control de calidad por nuestro equipo', desc: 'Muchos agentes subcontratan la inspección a quien nunca pisa la fábrica. Nosotros nos desplazamos para los proveedores en Taiwán; para China, la mercancía pasa por el almacén de nuestro agente de carga y la inspeccionamos antes de exportar. Fotos y video con cada envío.' },
        { title: 'Honestidad sobre el alcance', desc: 'No nos presentamos como veteranos en 50 países. Hoy tenemos una lista de proveedores activa en empaque, hogar y jardín. Para categorías cercanas lo decimos claramente y solo aceptamos el trabajo si podemos comprometernos a entregarlo.' },
      ],
      ctaTitle: 'Pruebe con un primer pedido pequeño',
      ctaDesc: 'Mínimo USD 1 000 por envío. Envíenos una imagen de referencia, cantidad objetivo y mercado de venta. Respondemos el mismo día con 2–3 direcciones de fábrica y el rango de precios esperado.',
      ctaBtn: 'Iniciar conversación de sourcing',
    },
    pt: {
