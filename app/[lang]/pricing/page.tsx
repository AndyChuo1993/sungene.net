...
export default async function Page({ params }: { params: Promise<{ lang: Lang }> }) {
  ...
  const plans = [
    {
      title: { zh: '名單買斷模式', en: 'Buyer List Model' },
      icon: <Database className="w-8 h-8 text-blue-600" />,
      color: 'blue',
      description: { zh: '適合已有業務團隊，需要高品質潛在客戶名單的企業。', en: 'Best for companies with sales teams needing high-quality leads.' },
      price: { zh: '依筆數計費', en: 'Pay Per Lead' },
      features: [
      ...
      cta: { zh: '索取名單報價', en: 'Get a Quote' },
      link: `/${lang}/contact?service=lead-generation`
    },
    {
      title: { zh: '專案開發模式', en: 'Outreach Project' },
      icon: <Send className="w-8 h-8 text-green-600" />,
      color: 'green',
      description: { zh: '適合想主動開發新市場，但缺乏開發信撰寫與系統經驗的企業。', en: 'Best for entering new markets without cold email expertise.' },
      price: { zh: '專案費用', en: 'Project Fee' },
      features: [
      ...
      <section className="bg-gray-900 text-white py-24 relative overflow-hidden">
        <div className="mx-auto max-w-7xl px-6 relative z-10 text-center">
          <div className="inline-block bg-yellow-500 text-gray-900 text-xs font-bold px-3 py-1 rounded-full uppercase tracking-wide mb-4">
             {lang === 'zh' ? '價格方案' : 'Pricing Plans'}
          </div>
          <h1 className="text-4xl md:text-5xl font-bold mb-6">
            {lang === 'zh' ? '透明、靈活的合作模式' : 'Transparent & Flexible Models'}
          </h1>
        ...
        <div className="mx-auto max-w-7xl px-6">
                    ...
                    <div key={index} className="bg-white rounded-2xl shadow-sm border border-gray-200 overflow-hidden hover:shadow-lg transition-shadow duration-300 flex flex-col">
                        <div className={`p-8 border-b border-gray-100 bg-${plan.color}-50/30`}>
                            ...
                            <h3 className="text-2xl font-bold text-gray-900 mb-2">
                                {lang === 'zh' ? plan.title.zh : plan.title.en}
                            </h3>
                            <p className="text-gray-600 text-sm min-h-[40px]">
                                {lang === 'zh' ? plan.description.zh : plan.description.en}
                            </p>
                        </div>
                        <div className="p-8 flex-grow">
                            <div className="mb-8">
                                <span className="text-3xl font-bold text-gray-900">
                                    {lang === 'zh' ? plan.price.zh : plan.price.en}
                                </span>
                            </div>
                        ...
                        <div className="p-8 pt-0 mt-auto">
                            <Link 
                                ...
                                `}
                            >
                                {lang === 'zh' ? plan.cta.zh : plan.cta.en}
                            </Link>
                        </div>
                    </div>
                ))}
            ...
            <div className="mt-16 text-center">
                <p className="text-gray-600 mb-4">
                    {lang === 'zh' ? '不確定哪種方案適合您？' : 'Not sure which plan is right for you?'}
                </p>
                <Link href={`/${lang}/contact`} className="text-blue-600 font-bold hover:underline">
                    {lang === 'zh' ? '聯繫我們討論客製化方案 →' : 'Contact us for a custom solution →'}
                </Link>
            </div>
        </div>
      </section>
