...
const introTx: Record<Lang, (q: string, opts: string, factors: string) => string> = {
  ...
  zh: (q, opts, factors) => `整理常見搜尋詞（${q}）與配置選項（${opts}），並把影響穩定性的關鍵點（${factors}）講清楚，方便你在申請報價前先把方向定下來。`,
  cn: (q, opts, factors) => `整理常见搜索词（${q}）与配置选项（${opts}），并把影响稳定性的关键点（${factors}）讲清楚，方便你在申请报价前先把方向定下来。`,
